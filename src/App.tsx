  ...
  return (
    <div style={card}>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
        <a style={btnGhost as any} href="#/">
          К списку
        </a>
        {perms?.assignBusinesses && (
          <button style={btnDanger} onClick={unassign}>
            Снять ответственного (в биржу)
          </button>
        )}
        {perms?.viewFinancials && (
          <a style={btnPrimary as any} href={`#/financials/${b.id}`}>
            Финансы
          </a>
        )}
        {perms?.viewFunnel && (
          <a style={btnPrimary as any} href={`#/funnel/${b.id}`}>
            Воронка
          </a>
        )}
      </div>
...
function Field(p: { label: string; children: React.ReactNode; span2?: boolean }) {
  return (
    <label style={{ display: 'block', gridColumn: p.span2 ? '1 / span 2' : undefined }}>
