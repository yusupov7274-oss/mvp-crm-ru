...
import OwnerDashboard from './dashboard/ui/OwnerDashboard';
...
function parseHash(): Route {
  ...
  if (a === 'dashboard') return { name: 'dashboard' };
  return { name: 'home' };
}
...
function BusinessCardScreen({ id }: { id: string }) {
  ...
  async function load() {
    setLoading(true);
    try { setB(await bizRepo.get(id)); } finally { setLoading(false); }
  }
  ...
  return (
    <div style={card}>
      ...
      <div style={{ display:'flex', gap:8, marginTop:8, flexWrap:'wrap' }}>
        ...
        {perms?.assignBusinesses && (<button style={btnDanger} onClick={unassign}>Снять ответственного (в биржу)</button>)}
        {perms?.viewFinancials && (<a style={btnPrimary as any} href={`#/financials/${b.id}`}>Финансы</a>)}
        {perms?.viewFunnel && (<a style={btnPrimary as any} href={`#/funnel/${b.id}`}>Воронка</a>)}
        {perms?.viewSummary && (<a style={btnPrimary as any} href={`#/summary/${b.id}`}>Сводная</a>)}
      </div>
    </div>
  );
}
...
/* ───────────── Навбар ───────────── */
function NavBar(){
  const { user, logout, perms } = useAuth() as any;
  return (
    <div style={topbar}>
      <div style={{display:'flex', gap:8, alignItems:'center'}}>
        <a href="#/" style={link}>Бизнесы</a>
        <a href="#/pool" style={link}>Биржа</a>
        {perms?.viewSummary && user?.role === 'owner' && (
          <a href="#/dashboard" style={link}>Дашборд</a>
        )}
      </div>
...
function Shell(){
  ...
  return (
    <div style={page}>
      ...
      {route.name === 'dashboard' && <OwnerDashboard />}
    </div>
  );
}
