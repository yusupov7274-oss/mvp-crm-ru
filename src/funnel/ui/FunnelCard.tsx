// UI: ввод воронки по месяцам, автоконверсии и выручка новых продаж

import React from 'react';
import { FunnelRecord } from '../types';
import * as repo from '../repo.local';
import { newFunnelRecord, compute } from '../calc';

export default function FunnelCard({ businessId, canEdit }: { businessId: string; canEdit: boolean }) {
  const [items, setItems] = React.useState<FunnelRecord[]>([]);
  ...
  async function addPeriod(){
    const rec = newFunnelRecord(businessId);
    const saved = await repo.upsert(rec);
    setItems(prev => [...prev, saved]);
  }

  async function patch(id: string, p: Partial<FunnelRecord>){
    if (!canEdit) return;
    const cur = items.find(x=>x.id===id)!;
    const next = { ...cur, ...p } as FunnelRecord;
          ...
          {items.map(r=>{
            ...
            return (
              <div key={r.id} style={panel}>
                ...
                <div style={grid3}>
                  <Num label="Лиды" v={r.leads} onChange={v=>patch(r.id,{leads:v})} dis={!canEdit}/>
                  <Num label="Количество встреч" v={r.meetings} onChange={v=>patch(r.id,{meetings:v})} dis={!canEdit}/>
                  <Num label="Количество продаж" v={r.sales} onChange={v=>patch(r.id,{sales:v})} dis={!canEdit}/>
                  <Num label="Средний чек новых продаж, ₽" v={r.avgCheck} onChange={v=>patch(r.id,{avgCheck:v})} dis={!canEdit}/>
                  <Num label="Исполненные обязательства" v={r.obligations} onChange={v=>patch(r.id,{obligations:v})} dis={!canEdit}/>
                </div>

                <div style={totals}>
                  <Badge>Конверсия встречи из лидов: <b>{c.convMeetFromLeads}%</b></Badge>
                  <Badge>Конверсия продажи из лидов: <b>{c.convSalesFromLeads}%</b></Badge>
                  <Badge>Конверсия продажи из встреч: <b>{c.convSalesFromMeet}%</b></Badge>
                  <Badge>Выручка новых продаж: <b>{fmt(c.newRevenue)}</b> ₽</Badge>
                </div>
              </div>
            );
          })}
...
function Num(p:{label:string; v:number; onChange:(v:number)=>void; dis?:boolean}){
  return <label style={{display:'block'}}>
    ...
    <input style={inp} type="number" value={p.v} onChange={e=>p.onChange(Number(e.target.value||0))} disabled={p.dis}/>
  </label>;
}
